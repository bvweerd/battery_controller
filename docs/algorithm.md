...
A planned **charge** is tested the same way, against the buy price: importing 1 kWh of AC stores `charge_eff(P_plan)` kWh, so a stored kWh costs `price / charge_eff + degradation`, and the plan is followed only while `λ ≥` that cost (±5 % hysteresis). This is what separates the two reasons the DP plans a charge. A cheap hour it deliberately buys in keeps λ above the cost of buying, so the import is executed as planned. A charge planned against forecast PV that did not arrive does not: λ then says the battery can be filled more cheaply later, and importing to keep the plan on paper is a loss. The fallback captures whatever surplus exists (zero-grid) or holds at `idle` when there is none — never grid import. Before this test the arbitration only ran while the meter showed export, which skipped exactly the case that needs it: the moment the charge starts importing (issue #174).

A **negative feed-in price** is the one case that keeps a commanded charge instead of falling back to zero-grid: exporting then costs money, and a setpoint that follows the meter deadlocks at ~0 W once the PV inverter curtails. It does not, however, license the import. The buy price is normally still positive, so the command is capped at the measured surplus (`battery − grid`) whenever λ does not justify buying the remainder — the plan runs on exactly what the sun delivers, and no more. Charging the surplus is self-correcting: the grid settles at ~0 W, and as PV recovers the surplus grows with it, so the setpoint climbs back to the plan on its own. Without the cap a charge planned on forecast PV stayed at full power through scattered cloud, importing at the buy price for the rest of the price period (issue #174).

Every hybrid branch measures the PV surplus as `battery − grid` rather than as the raw meter reading. The meter reads `load + battery − pv`, so it includes the battery's own action: a charge running on partial PV shows as import, and a zero-grid capture regulating the meter to ~0 W is indistinguishable from "no surplus left". Both made the surplus tests latch — once the battery acted, the reading no longer described the surplus it was acting on. `battery − grid` is `pv − load` by construction, so it is invariant to the setpoint and the hysteresis bands only have to damp sensor noise. The same measure drives the real-time `idle` → zero-grid upgrade, and the charge arbitration is repeated on every real-time tick (it is a handful of comparisons on the cached plan, no DP re-solve), so a failed PV forecast is corrected within seconds rather than at the next optimizer run.
