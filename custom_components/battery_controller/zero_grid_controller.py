...
class ZeroGridController:
    ...
    def _calculate_zero_grid(
        ...
        """
        # Target: opposite of grid power
        # If importing (grid > 0), discharge battery (negative power)
        # If exporting (grid < 0), charge battery (positive power)
        target_battery_w = -current_grid_w
