...
| **Battery power sensor** | Real-time battery charge/discharge power reading. **Positive = charging, negative = discharging** (opposite of the integration's output power sensors, which use positive = discharging). Enables accurate battery mode detection (charging/discharging/idle). | Battery mode defaults to "idle". Mode detection becomes unreliable. |
