...
class BatteryOptimalPowerSensor(BatteryControllerSensor):
    ...
    def native_value(self) -> float | None:
        if self.coordinator.data is None:
            return None
        return round(self.coordinator.data.get("optimal_power_kw", 0.0), 3)
...
class PVForecastSensor(BatteryControllerSensor):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        if result:
            attrs["forecast_kw"] = result.pv_forecast
...
class ConsumptionForecastSensor(BatteryControllerSensor):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        result = self._get_optimization_result()
        if result:
            return {"forecast_kw": result.consumption_forecast}
        return {}
...
class NetGridForecastSensor(BatteryControllerSensor):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        ...
        if result:
            net_load = [
                round(c - p, 3)
                for c, p in zip(result.consumption_forecast, result.pv_forecast)
            ]
            return {"forecast_kw": net_load}
        return {}
...
class BatteryGridSetpointSensor(BatteryControllerSensor):
    """Sensor for the battery grid power setpoint."""

    _attr_translation_key = "grid_setpoint"
    _attr_name = "Grid Setpoint"
    ...
    def __init__(self, coordinator, device, entry):
        super().__init__(coordinator, device, entry, "grid_setpoint")
